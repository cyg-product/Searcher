...
int main(void) {
    ...
    printf("SearchProtocol 구성:\n");
    printf("  메인페이지: %s\n", protocol.has_mainpage ? "있음" : "없음");
    printf("  검색 결과 페이지: %s\n", protocol.has_search_result ? "있음" : "없음");
    printf("  이미지 검색: %s\n", protocol.has_image_search ? "있음" : "없음");
    printf("  동영상 검색: %s\n", protocol.has_video_search ? "있음" : "없음");
    printf("  뉴스 검색: %s\n", protocol.has_news_search ? "있음" : "없음");
    printf("  로그인 기능: %s\n", protocol.has_login ? "있음" : "없음");
    printf("  기타 서비스: %s\n", protocol.has_extra_services ? "있음" : "없음");
